...
Доступ к GiftAPI предоставляется по запросу, напишите в бота поддержки в телеграм [@nfton_bot](http://t.me/nfton_bot) В сообщении укажите адрес вашего кошелька для которого вы создали API ключ, кратко поясните какие методы вам нужны и для чего.
